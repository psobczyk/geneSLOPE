...
When data is succesfully read, one can move to the second step of analysis.
...
```{r}
clumping.result <- clumping_procedure(screening.result, rho = 0.3, verbose = FALSE)
```
...
If one wants to identify interactively in which clump specific SNP is
...
The run function \emph{identify_clump} and select SNP of interest

```{r, eval=FALSE}
identify_clump(slope.result)
```

Then one can zoom into that clump
